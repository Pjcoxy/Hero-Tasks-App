...
function buildExtractionPrompt(email, kids, tz, nowIso) {
  ...
  return [
    ...
    '- personId: only when the email names one kid for something only they',
    '  can do - a fee for their class, their team, their form. Everything a',
    '  kid could choose to join is null: both kids are offered it and each',
    '  answers for themselves. Never guess from what an activity sounds like.',
    ...
    '  short tickable lines. One list per kid the item could be for: when',
    '  personId is null, repeat the same list for every kid - approval keeps',
    '  the list of whoever goes. points stays 0 - the parents decide what',
